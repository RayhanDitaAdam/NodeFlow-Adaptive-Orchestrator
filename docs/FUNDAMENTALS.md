...
- **`examples/`**: Contains example applications (e.g., `app.js`).
...
We don't use HTTP for local communication (like `gonode list`). Instead, we use a Unix Socket file (`/tmp/gonode.sock`). This is faster, more secure, and specialized for inter-process communication on the same server.
...
1. **Setup**: Run `./setup.sh` to install system dependencies (Go, Node, Nginx).
